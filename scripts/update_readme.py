...
USERNAME = os.environ.get("PROFILE_USERNAME", "CupoMeridio")
TOKEN = os.environ.get("GITHUB_TOKEN")
SVG_MODULES_PATH = BASE_DIR / "assets" / "dynamic_modules.svg"
SVG_TELEMETRY_PATH = BASE_DIR / "assets" / "live_telemetry.svg"
SVG_DIAGNOSTICS_PATH = BASE_DIR / "assets" / "diagnostics.svg"
...
def fetch_json(url: str):
    ...
    try:
        ...
        if e.code == 403:
            print("[!] Rate limit exceeded or access forbidden. Check GITHUB_TOKEN.", file=sys.stderr)
        raise
...
def generate_telemetry_svg():
    ...
    safe_uptime = html.escape(uptime, quote=True)
    safe_synced_at = html.escape(synced_at, quote=True)
  ...
  <text class="text" x="20" y="65">  uptime        :: <tspan class="accent pulse">{safe_uptime}</tspan></text>
  <text class="text" x="20" y="85">  last_push     :: <tspan class="accent pulse">{safe_repo}</tspan></text>
  <text class="text" x="20" y="105">  last_message  :: <tspan class="accent pulse">{safe_message}</tspan></text>
  <text class="text" x="20" y="125">  synced_at     :: <tspan class="accent pulse">{safe_synced_at}</tspan></text>
</svg>'''
    
    SVG_TELEMETRY_PATH.parent.mkdir(parents=True, exist_ok=True)
    SVG_TELEMETRY_PATH.write_text(svg_content, encoding="utf-8")
...
'''
  ...
  <path class="bg" d="M0 0 H{svg_width} V{svg_height - 8} A8 8 0 0 1 {svg_width - 8} {svg_height} H8 A8 8 0 0 1 0 {svg_height - 8} Z" />
...
'''
    ...
    for i, (lang, pct) in enumerate(top_langs):
        ...
        svg_content += f'''
  <text class="text lang-{i}" x="20" y="{y + 14}">{safe_lang}</text>
...
'''
    ...
    SVG_MODULES_PATH.parent.mkdir(parents=True, exist_ok=True)
    SVG_MODULES_PATH.write_text(svg_content, encoding="utf-8")
...
def get_user_diagnostics():
    try:
        ...
        print(f"[!] Impossibile recuperare la diagnostica utente: {e}", file=sys.stderr)
...
def generate_diagnostics_svg():
    ...
    safe_repos = html.escape(str(stats['repos']), quote=True)
    safe_stars = html.escape(str(stats['stars']), quote=True)
    safe_followers = html.escape(str(stats['followers']), quote=True)
    safe_following = html.escape(str(stats['following']), quote=True)
  ...
  <text class="text l2" x="20" y="60" xml:space="preserve">    DATA.REPOSITORIES <tspan class="hl">></tspan>  <tspan class="{val_class}">{safe_repos}</tspan> DEPLOYED</text>
  <text class="text l3" x="20" y="85" xml:space="preserve">    DATA.STARGAZERS   <tspan class="hl">></tspan>  <tspan class="{val_class}">{safe_stars}</tspan> DETECTED</text>
  <text class="text l4" x="20" y="110" xml:space="preserve">    DATA.NETWORK      <tspan class="hl">></tspan>  <tspan class="{val_class}">{safe_followers}</tspan> FOLLOWERS, <tspan class="{val_class}">{safe_following}</tspan> FOLLOWING</text>
</svg>'''

    SVG_DIAGNOSTICS_PATH.parent.mkdir(parents=True, exist_ok=True)
    SVG_DIAGNOSTICS_PATH.write_text(svg_content, encoding="utf-8")
    print(f"[✓] {SVG_DIAGNOSTICS_PATH} generato con successo.")
